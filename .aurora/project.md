...
- Aurora planning infrastructure
...
- Plan ID format: `NNNN-slug` or `verb-noun-slug` (kebab-case)
- Four-file workflow: `plan.md`, `tasks.md`, `design.md` (optional), `specs/`
- Directory structure: `.aurora/plans/active/` and `.aurora/plans/archive/`

### Testing Strategy

**Test Coverage**: >=95% for all packages
...
- `tests/e2e/` - End-to-end tests for real scenarios
...
- Slash commands: `/aur:plan`, `/aur:search`, `/aur:get`, `/aur:archive`
...
**Project Root**:
- `.aurora/` - Project-level Aurora directory
- `.aurora/AGENTS.md` - AI assistant instructions
- `.aurora/project.md` - Project context (this file)
- `.aurora/specs/` - Capability specifications
- `.aurora/plans/` - Active and archived plans
...
**Commands**:
- `aur init` - Initialize Aurora for project
- `aur mem index .` - Index codebase
- `aur mem search "query"` - Semantic search
- `aur goals "goal"` - Decompose goal into tasks
