...
def benchmark_deferred_init():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"  This includes: directory check, thread-local setup")
        print(f"  Schema init is deferred until first DB operation")
        ...
        print(f"  This includes: connection pool lookup, schema init, query")

        print(f"\nTotal average (init + first access): {avg_init + avg_access:.3f}ms")
        print(f"  Without deferred init, this would all happen in __init__")
