...
**Purpose**: Test every CLI command and option
...
## Command 2: `aur mem` - Memory Management

These commands are local database operations.
...
## Command 3: `aur query` - Query Execution
...
# ✓ Shows configuration (model, provider)
...
- [ ] `aur --help` - Show help
- [ ] `aur init` - Create config
- [ ] `aur mem index <path>` - Index Python files
- [ ] `aur mem search <query>` - Search indexed code
- [ ] `aur mem stats` - Show memory stats
