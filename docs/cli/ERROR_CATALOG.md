...
- API key in config file is incorrect
- Network preventing authentication
...
5. **Verify API key format**:
   ```bash
   # Valid format: sk-ant-<identifier>
...
### ERR-005: Missing API Key

**Message**:
```
[Config] ANTHROPIC_API_KEY not found.
Cause: AURORA needs an API key to connect to LLM.
```

**Cause**:
- API key not set in environment variable
- API key not set in config file
- Config file doesn't exist
- First time using AURORA
...
**Security Note**: Never commit API keys to version control or share publicly.

**Related**: ERR-001, ERR-006
