...
# Should show: API Key: sk-ant-...xyz ✓
```
...
3. **Set correctly**:
   ```bash
   # Don't use quotes that include newlines
...
4. **Set budget limit** (headless mode):
   ```bash
   aur headless experiment.md --budget 2.0
   ```
...
export ANTHROPIC_API_KEY=sk-ant-...
aur query "test"
# Still shows: ANTHROPIC_API_KEY not found
```
...
1. **Export variable**:
   ```bash
   # Without export, variable not inherited
   export ANTHROPIC_API_KEY=sk-ant-...
   ```
...
3. **Set in command**:
   ```bash
   ANTHROPIC_API_KEY=sk-ant-... aur query "test"
   ```
...
docker run -e ANTHROPIC_API_KEY=sk-ant-... aurora
...
# Check DNS
docker run aurora ping api.anthropic.com
```
...
echo -e "\n=== Environment ==="
env | grep AURORA
env | grep ANTHROPIC | sed 's/\(ANTHROPIC_API_KEY=\).*/\1[REDACTED]/'
```
