...
Commands:
  headless  Run AURORA in headless mode with a goal file.
...
#### 3. Headless Mode (`aur headless`)

Run AURORA autonomously with a goal file:

```bash
# Run headless mode with goal file
aur headless goal.md

# With safety constraints
aur headless goal.md --enforce-branch feature-x --budget-limit 1000

# Dry run (show plan without executing)
aur headless goal.md --dry-run
```

**Safety Features**:
- `--enforce-branch` - Require specific git branch (prevents accidental changes on main)
- `--budget-limit` - Set maximum token budget
- `--max-iterations` - Cap number of reasoning cycles
- `--dry-run` - Preview execution plan without making changes

**Goal File Format** (Markdown):
```markdown
...
## Testing Workflow After NumPy Fix
...
# Headless mode
aur headless goal.md [--enforce-branch BRANCH] [--budget-limit N]
...
- **Headless Mode**: Varies by goal complexity
