...
class KeywordExtractor:
    ...
    STOP_WORDS = {
        ...
        'do', 'does', 'did', 'should', 'could', 'would', 'may', 'might',
        'must', 'shall', 'not', 'no', 'nor', 'if', 'then', 'so', 'than',
        'too', 'very', 'just', 'also', 'only', 'both', 'either', 'neither',
    }

    # Programming-specific terms that should NOT be filtered
    PROGRAMMING_TERMS = {
        'api', 'app', 'aws', 'cli', 'cpu', 'csv', 'css', 'db', 'dev',
        'dto', 'end', 'env', 'etc', 'git', 'gpu', 'gui', 'html', 'http',
        'id', 'ide', 'io', 'ip', 'jar', 'js', 'json', 'jwt', 'key',
        'lib', 'log', 'mac', 'max', 'min', 'npm', 'null', 'os', 'pdf',
        'php', 'png', 'py', 'ram', 'ref', 'rgb', 'row', 'run', 'sdk',
        'sql', 'ssh', 'ssl', 'std', 'svg', 'tcp', 'tls', 'tmp', 'ttl',
...
class ContextBoost:
    ...
    def calculate_from_chunk_fields(
        ...
        docstring_keywords = (
            self.extractor.extract(chunk_docstring)
            if chunk_docstring else set()
        )
        signature_keywords = (
            self.extractor.extract(chunk_signature)
            if chunk_signature else set()
        )
        body_keywords = (
            self.extractor.extract(chunk_body)
            if chunk_body else set()
        )
    ...
    def explain_boost(
        ...
        overlap_fraction = (
            len(matching) / len(query_keywords)
