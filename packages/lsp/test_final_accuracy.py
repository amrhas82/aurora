...
TEST_CASES = [
    ...
    ("packages/core/src/aurora_core/exceptions.py", "ChunkNotFoundError", 80, 6, "Storage exception"),
    ...
    ("packages/soar/src/aurora_soar/orchestrator.py", "SoarOrchestrator", 89, 6, "Main orchestrator"),
]
...
def grep_semantic_count(symbol: str) -> tuple[int, int, int]:
    ...
    try:
        ...
        result3 = subprocess.run(
            ["grep", "-rw", "--include=*.py", "-E", f"^(from|import).*{symbol}", str(WORKSPACE / "packages")],
            capture_output=True, text=True
        )
...
async def main():
    ...
    try:
        for filepath, symbol, line, col, desc in TEST_CASES:
            ...
            print(f"   GREP:  {grep_total:3} total ({grep_usages} usages + {grep_imports} imports) in {grep_files} files")
            print(f"   LSP:   {len(refs):3} total ({lsp_usages} usages + {lsp_imports} imports) in {lsp_files} files")
        ...
        totals = {"grep": 0, "grep_use": 0, "grep_imp": 0, 
                  "lsp": 0, "lsp_use": 0, "lsp_imp": 0}
