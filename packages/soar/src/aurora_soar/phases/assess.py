...
class ComplexityAssessor:
    ...
    SIMPLE_VERBS = {
        'what', 'show', 'list', 'get', 'find', 'print', 'check', 'read',
        'open', 'run', 'where', 'which', 'display', 'view', 'see', 'tell',
        'give', 'name', 'count', 'who', 'when', 'is'
    }

    # Medium action verbs - require moderate work (add score)
    MEDIUM_VERBS = {
        'add', 'update', 'fix', 'write', 'change', 'modify', 'remove', 'delete',
        'improve', 'enhance', 'extend', 'convert', 'rename', 'move', 'test',
        'configure', 'setup', 'set', 'enable', 'disable'
    }

    # Analysis keywords - require understanding/reasoning (add score)
    ANALYSIS_VERBS = {
        'explain', 'compare', 'analyze', 'debug', 'understand', 'investigate',
        'describe', 'clarify', 'elaborate', 'why', 'difference', 'mean',
        'interpret', 'evaluate', 'assess', 'review', 'examine', 'diagnose',
        'trace', 'audit'
    }

    # Complex action keywords - require significant work (large score boost)
    COMPLEX_VERBS = {
        'implement', 'design', 'architect', 'refactor', 'integrate', 'migrate',
        'build', 'create', 'develop', 'construct', 'engineer', 'establish',
        'transform', 'overhaul', 'rewrite', 'restructure', 'optimize'
    }

    # Scope expansion keywords - broaden the request (significant boost)
    SCOPE_KEYWORDS = {
        'all', 'every', 'entire', 'across', 'comprehensive', 'complete',
        'codebase', 'project', 'system', 'application', 'full', 'whole',
        'everything', 'throughout', 'universal', 'global'
    }

    # Constraint phrases - add conditions/requirements
    CONSTRAINT_PHRASES = [
        'without breaking', 'without changing', 'maintaining', 'ensuring',
        'while also', 'while keeping', 'make sure', 'must not', 'should not',
        'backwards compatible', 'backward compatible', 'don\'t break',
        'keep existing', 'preserve', 'without affecting', 'without modifying',
        'must work with', 'keeping it', 'while still'
    ]

    # Multi-step indicators - sequential operations
    SEQUENCE_MARKERS = [
        'first', 'then', 'after that', 'finally', 'next', 'afterwards',
        'subsequently', 'step by step', 'following that', 'once done',
        'before', 'prior to', 'and then'
    ]

    # Compound requirement markers
    COMPOUND_MARKERS = [
        'and also', 'as well as', 'additionally', 'furthermore', 'moreover',
        'in addition', 'along with', 'together with', 'plus', 'not only'
    ]

    # Technical domain keywords (cross-cutting concerns)
    TECHNICAL_DOMAINS = {
        'security', 'performance', 'scalability', 'reliability', 'testing',
        'authentication', 'authorization', 'caching', 'logging', 'monitoring',
        'database', 'api', 'frontend', 'backend', 'infrastructure', 'deployment',
        'ci/cd', 'docker', 'kubernetes', 'microservices', 'distributed'
    }

    # Feature/system nouns that increase complexity when combined with action verbs
    COMPLEX_NOUNS = {
        'authentication', 'authorization', 'oauth', 'jwt', 'session', 'sessions',
        'pipeline', 'workflow', 'notification', 'notifications', 'dashboard',
        'crud', 'plugin', 'framework', 'websocket', 'websockets', 'realtime',
        'real-time', 'rate-limit', 'rate-limiting', 'pagination', 'search',
        'validation', 'migration', 'schema'
    }

    # Question type patterns
    SIMPLE_QUESTION_PATTERNS = [
        r'^what is\b', r'^where is\b', r'^which\b', r'^who\b',
        r'^is there\b', r'^does it\b', r'^can i\b'
    ]
    ...
    COMPLEX_QUESTION_PATTERNS = [
        ...
        r'\bbest\s+architecture\b', r'\bdesign.*pattern\b',
    ...
    def _detect_critical(self, prompt_lower: str) -> bool:
        ...
        tier1_keywords = {
            'emergency', 'outage', 'breach', 'vulnerability', 'exploit',
            'corruption', 'data loss', 'incident', 'penetration'
        }
        ...
        critical_actions = {'fix', 'patch', 'investigate', 'secure', 'protect',
                           'mitigate', 'prevent', 'respond', 'handle'}

        has_tier2 = any(re.search(r'\b' + re.escape(k) + r'\b', prompt_lower)
                       for k in tier2_keywords)
        has_action = any(re.search(r'\b' + re.escape(a) + r'\b', prompt_lower)
        ...
        has_financial = any(re.search(r'\b' + re.escape(k) + r'\b', prompt_lower)
                          for k in financial_keywords)
        has_security = any(re.search(r'\b' + re.escape(k) + r'\b', prompt_lower)
                         for k in security_context)
    ...
    def _score_keywords(self, prompt_lower: str) -> tuple[int, list[str]]:
        ...
        if re.search(r'\bfollowing\s+\w+\s*(?:guidelines?|standards?|practices?|rules?)\b', prompt_lower):
            score += 10
            signals.append('standards_compliance')

        # Bounded scope detection (reduces complex verb impact)
        if complex_matches and re.search(r'\b(?:this|the|a)\s+(?:function|method|class|component|file)\b', prompt_lower):
            if word_count <= 8:
                score -= 10
    ...
    def _score_scope(self, prompt_lower: str) -> tuple[int, list[str]]:
        ...
        # Directory patterns
        dir_refs = re.findall(r'\b(?:src|lib|tests?|docs?|components?|modules?|packages?)/\w+', prompt_lower)
        if dir_refs:
    ...
    def _score_constraints(self, prompt_lower: str) -> tuple[int, list[str]]:
        ...
        negatives = len(re.findall(r'\b(?:don\'t|dont|do not|never|avoid|shouldn\'t|must not)\b', prompt_lower))
        if negatives > 0:
            score += negatives * 6
    ...
    def _score_structure(self, prompt: str) -> tuple[int, list[str]]:
        ...
        # Success criteria
        if re.search(r'\b(?:should|must|needs? to|has to)\s+(?:result|return|output|produce|work|be)\b', prompt.lower()):
            score += 8
            signals.append('success_criteria')

        # Conditional logic
        conditionals = len(re.findall(r'\bif\b.*\bthen\b|\bwhen\b.*\bshould\b|\bdepending on\b', prompt.lower()))
        if conditionals > 0:
    ...
    def _score_domain(self, prompt_lower: str) -> tuple[int, list[str]]:
        ...
        frameworks = re.findall(r'\b(?:react|vue|angular|django|flask|fastapi|express|spring|rails)\b', prompt_lower)
        if frameworks:
    ...
    def _calculate_multiplier(self, signals: list[str]) -> float:
        ...
        complex_indicators = sum(1 for s in signals if any(x in s for x in [
            'complex_verbs', 'scope_expansion', 'constraint', 'multi_domain',
            'sequence_markers', 'compound', 'numbered_list'
        ]))

        # Count simplicity indicators
        simple_indicators = sum(1 for s in signals if any(x in s for x in [
            'simple_verbs', 'simple_question'
        ]))
