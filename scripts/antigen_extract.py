...
def analyze_bad_session(session_file, analysis, signals):
    ...
    anchor_signals = ['user_intervention', 'session_abandoned', 'false_success', 'interrupt_cascade']
    anchors = [s for s in signals if s.get('session') == session_id and s.get('signal') in anchor_signals]
    ...
    for anchor in anchors:
        ...
        # Remove common words
        common = {'this', 'that', 'with', 'from', 'have', 'what', 'when', 'where', 'which', 'there', 'their', 'would', 'could', 'should', 'about', 'been', 'were', 'they', 'them', 'then', 'than', 'these', 'those', 'some', 'into', 'only', 'other', 'also', 'just', 'more', 'very', 'here', 'after', 'before', 'being', 'doing', 'make', 'made', 'like', 'want', 'need', 'file', 'code'}
        keywords -= common
...
def main():
    ...
    if not analysis_file.exists():
        print('Error: Run friction_analyze.py first to generate .aurora/friction/friction_analysis.json')
        return 1
