...
def load_config(config_path=None):
    """Load weights from JSON config, falling back to embedded config."""
...
def extract_signals(session_file):
    ...
    for ts, msg_key in user_messages:
        if msg_key in seen_messages and len(msg_key) > 20:  # Ignore very short messages
            signals.append(
                {"ts": ts, "source": "user", "signal": "repeated_question", "details": msg_key[:50]}
            )
        ...
        if content is not None:
            ...
            if isinstance(content, list):
                for block in content:
                    if isinstance(block, dict) and block.get("type") == "tool_result":
                        ...
                        if re.search(
                            r"Exit code 137|Request interrupted|interrupted by user", result, re.I
                        ):
                            signals.append(
                                {
                                    "ts": ts,
                        ...
                        # Check for sibling tool call errors (cascade from failed parallel execution)
                            ...
                            r"<tool_use_error>Sibling tool call errored</tool_use_error>",
                            result,
                            re.I,
                        ):
                            signals.append(
                        ...
                        # Match non-zero exit codes or Python tracebacks (excluding 137 which is interruption)
                            ...
                            re.search(r"Exit code [1-9]", result) and "Exit code 137" not in result
                        ) or re.search(r"Traceback \(most recent|CalledProcessError", result):
                            signals.append(
                                {
                                    "ts": ts,
                                    "source": "tool",
            ...
            if isinstance(content, dict) and content.get("type") == "tool_result":
                ...
                if re.search(r"Exit code [1-9]|Traceback \(most recent|CalledProcessError", result):
                    ...
                    signals.append(
                        {"ts": ts, "source": "tool", "signal": "exit_success", "details": ""}
                    )
            ...
            if isinstance(content, str):
                ...
                if is_interactive and re.search(
                    r"\b(no|didn\'t work|still broken)\b", content, re.I
                ):
                    signals.append(
                        {
                            "ts": ts,
...
def analyze_session(session_id, signals, metadata, config):
    ...
    for i in range(len(signal_seq) - 2):
        ...
        if seq == ("exit_error", "false_success", "user_curse"):
            patterns.append({"type": "false_success_loop", "sequence": list(seq), "count": 1})
...
def aggregate_sessions(analyses, config):
    ...
    aggregate_by_source = defaultdict(
        lambda: {"sessions_with_signals": 0, "total_friction": 0, "top_signals": Counter()}
    )
    ...
    total_objective = aggregate_by_source.get("tool", {}).get(
        "total_friction", 0
    ) + aggregate_by_source.get("user", {}).get("total_friction", 0)
    total_llm = aggregate_by_source.get("llm", {}).get(
        "total_friction", 1
    ...
    if snr < thresholds["signal_noise_ratio"]:
        status = "BLOAT"
        reasons.append(
            f'Signal/noise ratio: {snr:.1f} (threshold: {thresholds["signal_noise_ratio"]})'
        )
    elif intervention_pred < thresholds["intervention_predictability"]:
        status = "INCONCLUSIVE"
        reasons.append(
            f'Intervention predictability: {intervention_pred:.0%} (threshold: {thresholds["intervention_predictability"]:.0%})'
        )
    else:
        status = "USEFUL"
        reasons.append(
        ...
        # Recommendations
        user_curses = (
            aggregate_by_source.get("user", {}).get("top_signals", {}).get("user_curse", 0)
        )
        if user_curses > 5:
            actions.append("Consider increasing user_curse weight (high occurrence)")

        false_success_loops = len(
            [p for p in all_patterns if p.get("type") == "false_success_loop"]
        )
    ...
    for source, data in aggregate_by_source.items():
        ...
        aggregate_by_source_dict[source] = {
            ...
            "total_friction": data["total_friction"],
            "avg_friction_per_session": (
                data["total_friction"] / sessions_count if sessions_count > 0 else 0
            ),
    ...
    interactive_analyses = [
        a for a in analyses if a.get("session_metadata", {}).get("turn_count", 0) > 1
    ]
    ...
    if interactive_analyses:
        ...
        worst_session = max(
            interactive_analyses, key=lambda x: x.get("friction_summary", {}).get("peak", 0)
        )
        ...
        if ok_sessions:
            best_session = min(
                ok_sessions, key=lambda x: x.get("friction_summary", {}).get("peak", 0)
            )
        else:
            best_session = min(
                interactive_analyses, key=lambda x: x.get("friction_summary", {}).get("peak", 0)
    ...
    return {
        ...
        "verdict": {"status": status, "reasons": reasons, "recommended_actions": actions},
    }
...
def generate_detailed_report(output_dir, analyses, summary, config, signal_counts, multi_project):
    ...
    report.append(
        f'**Interactive Sessions:** {summary["overall"]["interactive_sessions"]} (multi-turn conversations)\n'
    )
    report.append(
        f'**BAD Sessions:** {summary["overall"]["bad_sessions"]} ({summary["overall"]["bad_rate"]*100:.0f}% of interactive)\n\n'
    )
    ...
    if bad_rate > 0.5:
        report.append(
            f"⚠️  **CRITICAL**: {bad_rate*100:.0f}% of interactive sessions end in failure. "
        )
    elif bad_rate > 0.3:
        report.append(
            f"🟡 **WARNING**: {bad_rate*100:.0f}% of interactive sessions end in failure. "
        )
    else:
        report.append(
            f"✅ **HEALTHY**: {bad_rate*100:.0f}% of interactive sessions end in failure. "
        )

    report.append(
        f'Average session: {overall["avg_turns"]:.1f} turns, {overall["avg_friction"]:.1f} friction, {overall["avg_duration_min"]:.0f} min.\n\n'
    )
    ...
    if false_success_count > 0:
        report.append(
            f"**False Success Loop** ({false_success_count} occurrences): LLM claims task is complete after command fails. "
        )
        report.append("This indicates the LLM is not checking exit codes properly.\n\n")

    if exit_error_count > 50:
        report.append(
            f"**High Error Rate** ({exit_error_count} errors): Many commands are failing. "
        )
        report.append(
            "This suggests either environment issues or LLM choosing wrong approaches.\n\n"
        )

    if interrupt_count > 20:
        report.append(
            f"**User Interruptions** ({interrupt_count} interrupts): Users frequently canceling operations. "
        )
    ...
    if intervention_count > 0:
        ...
        report.append(
            f'**Abandonment Rate** ({intervention_rate*100:.0f}%): {intervention_count}/{summary["overall"]["interactive_sessions"]} interactive sessions ended with user giving up. '
        )
    ...
    report.append(
        f"**Low Friction (0-15):** {len(low_friction)} sessions - Normal operation, minor errors quickly resolved\n\n"
    )
    report.append(
        f"**Medium Friction (15-50):** {len(medium_friction)} sessions - Some struggles, multiple retries, but eventually successful\n\n"
    )
    report.append(
        f"**High Friction (50+):** {len(high_friction)} sessions - Severe issues, user frustration, likely gave up\n\n"
    )
    ...
    for a in high_friction:
        ...
        if multi_project:
            report.append(
                f"| {project} | {sid} | {quality} | {peak} | {turns} | {dur_str} | {sigs_str} |\n"
            )
        else:
    ...
    if "by_project" in summary and summary["by_project"]:
        ...
        for proj, stats in sorted(summary["by_project"].items()):
            bad_rate_pct = (
                f"{stats['bad_rate']*100:.0f}%" if stats["interactive_sessions"] > 0 else "-"
            )
            dur = stats.get("avg_duration_min", 0)
            dur_str = format_duration(int(dur)) if dur else "-"
            report.append(
                f'| {proj} | {stats["interactive_sessions"]} | {stats["bad_sessions"]} | {bad_rate_pct} | {stats["avg_friction"]:.1f} | {stats["avg_turns"]:.1f} | {dur_str} |\n'
            )
        report.append("\n")
    ...
    if false_success_count > 10:
        recommendations.append(
            "**High Priority:** Add CLAUDE.md rule to verify exit codes before claiming success"
        )

    if interrupt_count > 20:
        recommendations.append(
            "**High Priority:** Commands timing out or stuck - review for heavy operations that need optimization"
        )

    if signal_counts.get("tool_loop", 0) > 3:
        recommendations.append(
            "**Medium Priority:** Add CLAUDE.md rule to detect and break out of tool loops"
        )

    if intervention_count / summary["overall"]["interactive_sessions"] > 0.4:
        recommendations.append(
            "**Critical:** >40% abandonment rate - major UX issues, review antigens for patterns"
        )

    if signal_counts.get("repeated_question", 0) > 20:
        recommendations.append(
            "**Medium Priority:** Many repeated questions - LLM not understanding user intent or context issues"
        )
    ...
    if "daily_stats" in summary and summary["daily_stats"]:
        ...
        for day in summary["daily_stats"][-14:]:
            ...
            report.append(
                f'| {day["date"]} | {day["interactive"]} | {day["bad"]} | {bad_rate_pct} | {bar} |\n'
            )
        report.append("\n")
...
def main():
    ...
    if input_path.is_file():
        ...
        session_files = [f for f in input_path.glob("*.jsonl") if "sessions-index" not in f.name]

        # If no direct session files, check if this is a parent directory with project subdirs
        if not session_files:
            project_dirs = [
                d for d in input_path.iterdir() if d.is_dir() and not d.name.startswith(".")
            ]
                ...
                if proj_sessions:
                    ...
                    for pd in project_dirs:
                        session_files.extend(
                            [f for f in pd.glob("*.jsonl") if "sessions-index" not in f.name]
                        )
                    break
    ...
    if "by_project" in summary and summary["by_project"]:
        ...
        for proj, stats in sorted(summary["by_project"].items()):
            ...
            interactive_sessions = [
                a for a in proj_sessions if a["session_metadata"].get("turn_count", 0) > 1
            ]
            if interactive_sessions:
                ...
                print(
                    f'  {proj:12s} {bad_pct:>4s} BAD ({stats["bad_sessions"]}/{stats["interactive_sessions"]})  median: {median:.1f}  {emoji}'
                )
            else:
                print(
                    f'  {proj:12s} {bad_pct:>4s} BAD ({stats["bad_sessions"]}/{stats["interactive_sessions"]})  {emoji}'
                )
        print()
    ...
    if "daily_stats" in summary and summary["daily_stats"]:
        ...
        for day in summary["daily_stats"][-14:]:
            ...
            bar = "█" * bar_len + "░" * (10 - bar_len)
            print(
                f'  {day["date"]}  {day["interactive"]:2d} sessions  {day["bad"]:2d} BAD  {bar}  {bad_rate*100:.0f}%'
            )
        print()
